...
export function toGraphQLDefinition(thinkyModel, opts = {}) {
  ...
  opts.map = opts.map   || {};
  opts.globalId = (opts.globalId !== undefined) ? opts.globalId : false;
  opts.allowNull = (opts.allowNull !== undefined) ? opts.allowNull : true;
  ...
  const graphQLDefinition = Object.keys(modelSchema).reduce((gqlDef,key) => {
    ...
    if (opts.exclude.indexOf(key) !== -1) return gqlDef;            // Skip Excluded
    if (opts.only && opts.only.indexOf(key) === -1) return gqlDef; // Only those fields
  ...
  if (opts.globalId) {
    ...
    graphQLDefinition[modelName.toLowerCase() + 'ID'] = {
      type: GraphQLString,
      resolve: (source) => {
        return source.id;
      }
...
export function attributeToGraphQLType(attributeType,name) {
  ...
  if (customTypeMapper) {
    let result = customTypeMapper(attributeType);
  ...
  // Bolean Type
  if (type.isBoolean(attributeType)) return GraphQLBoolean;
  ...
  if (type.isObject(attributeType)) {
    ...
    if (!name) throw Error('Specify a name for the Object Attribute type');
